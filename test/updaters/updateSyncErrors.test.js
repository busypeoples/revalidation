...
describe('updaters/updateSyncErrors', () => {
  ...
  it('should return an empty array when field value is valid and validateSingle is set to true', () => {
    ...
    const result = updateSyncErrors([{form: {name: 'foobar'}, errors: {}}, []], UPDATE_FIELD, {rules, validateSingle: true, name: 'name'})
  ...
  it('should return an array containing the error messages when field value is invalid', () => {
    ...
    const result = updateSyncErrors([{form: {name: 'bar'}, errors: {}}, []], UPDATE_FIELD, {rules, validateSingle: true, name: 'name'})
    deepEqual(expected, result)
  })
  ...
  it('should skip validating when validateSingle is set to false and a field has been updated', () => {
    ...
    const result = updateSyncErrors([{form: {name: 'foo'}, errors: {}}, []], UPDATE_FIELD, {rules, validateSingle: false, name: 'name'})
    deepEqual(expected, result)
  })
  ...
  it('should return an empty array when field value is valid and validateSingle is set to true and a validate field has been triggered', () => {
    const expected = [{form: {name: 'foobar'}, errors: {name: []}}, []]
    const result = updateSyncErrors([{form: {name: 'foobar'}, errors: {}}, []], VALIDATE_FIELD, {rules, validateSingle: true, name: 'name'})
    deepEqual(expected, result)
  })

  it('should validate all fields when the complete form state has been updated', () => {
    const expected = [{form: {name: 'foo', random: 'random'}, errors: {name: ['Minimum length is four.'], random: ['Minimum length is seven.']}}, []]
    const result = updateSyncErrors([{form: {name: 'foo', random: 'random'}, errors: {}}, []], UPDATE_ALL, {rules})
    deepEqual(expected, result)
  })

  it('should validate all fields a validate all actions has been triggered', () => {
    const expected = [{form: {name: 'foo', random: 'random'}, errors: {name: ['Minimum length is four.'], random: ['Minimum length is seven.']}}, []]
    const result = updateSyncErrors([{form: {name: 'foo', random: 'random'}, errors: {}}, []], VALIDATE_ALL, {rules})
    deepEqual(expected, result)
  })
